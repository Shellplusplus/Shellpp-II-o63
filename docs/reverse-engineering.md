...
当前最小 Activity/LVX 页面创建链已经恢复并用于显示固定文本，但完整业务 UI、系统通知真实提交 ABI、无重启删除 ABI、图标节点公共结构及所有通用 callback 签名仍未完全恢复。旧 10 Pro 地址不可迁移。字符串地址只是 literal/data 引用，不能直接当作函数入口。
